...
// !!!!!! NO OLVIDAR ¡¡¡¡¡¡ Antes de compilar Seleccionar Reloj
/*
...
// lineas : 761, 762
// rtc.adjust (DateTime (Fa, Fm, Fd,  Rh, Rm, 0)); // Graba los valores en el RTCC 
// contador=0;                                     // Vuelve todo al comienzo una vez grabado 

// Quitada la grabacion de EEPROM
    /*      
    Fd = EEPROM.read (37);       // no hay que guaradar en eeprom algo que varia constantemente como la hora 
    Fm = EEPROM.read (38);       // si los timbres ya que estop se programan y quedan en memoria la hora
    Fa = EEPROM.read (39);       // se toma del propio reloj no de la eeprom 
    Rh = EEPROM.read (40);
    Rm = EEPROM.read (41);  
    */  
...
// Igualé las variables que se exhiben por display 
/*
...
// Comenté todo esto ya no hace falta 
/*
//Serial.println("Estableciendo Hora y fecha...");
  ...
  // esto que sigue ya no tiene sentido lo comento
...
void setup () {
  ...
  pinMode(salida_t,OUTPUT);             // Salida de timbre 
...
void loop () {
  ...
  { // 1000 mseg  equivale a 1 segundo, 250 = 1/4 segundo
    ...
    cuarto_seg++; // Variable para usar en la duración del timbre 
