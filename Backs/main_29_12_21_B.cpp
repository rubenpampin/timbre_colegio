...
void loop () {
  ...
  if (t_actual - t_ant > 500)   // compara si el valor actual con el anterior mas un tiempo en milisegundos
...
if (muestra == 2) {   // si pasaron 5 segundos va a entrar en la rutina de mostrar y quito los retardos de 6 segundos
 ...
 switch (contador)
 {
  ...
  case 1:
      ...
      lcd.setCursor ( 1, 0 ); // Mover el cursor a la fila (1) columna (0)
      lcd.print("T3:"); // Imprime en Display "Timbre1:"
      lcd.print(T3h);
      lcd.print(T3m);

      lcd.setCursor(1,8);  // Mover el cursor a la fila (1) columna (0)
